...
const JobUpdate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Optional: trigger a notification insert
    await supabase.from('notifications').insert([{
      user_id: job.client_id,
      message: `Job "${job.propertyName}" was updated.`,
      job_id: jobId,
      type: 'job_update',
      seen: false,
    }]);
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-xl shadow">
      <h2 className="text-2xl font-bold text-blue-700 mb-4">🔧 Update Job: {job.propertyName}</h2>
